...
Synology NoteStation에 노트가 2,000개 넘어가면 검색이 안 된다. 키워드 하나 틀리면 못 찾고, 비슷한 주제로 묶어 보는 건 불가능하다. 분명 어딘가 적어뒀는데 어디에 있는지 모른다.
...
**검색이 진짜로 된다**
FTS + 퍼지 + 시맨틱 검색을 하나의 PostgreSQL에서 돌린다. 키워드 틀려도 찾고, 의미가 비슷하면 찾는다. 왜 이 결과가 나왔는지도 보여준다.
...
**자연어로 질문한다**
"온도에 따른 효소 활성 변화" — 키워드가 아니라 질문으로 검색한다. AI 사서가 2,000개 노트에서 관련도 순으로 찾아준다.
...
**하이브리드 검색** — tsvector(BM25) + pg_trgm(퍼지) + pgvector(시맨틱)를 Reciprocal Rank Fusion으로 병합한다. 별도 벡터 DB 없이 PostgreSQL 하나로 끝난다. 결과가 충분하면 임베딩 호출을 자동으로 건너뛴다(Adaptive Search).

**AI 프로바이더 4종** — OpenAI, Anthropic, Google, ZhipuAI. 환경 변수에 API 키를 넣으면 자동 감지한다. 원하는 모델로 자유 전환. SSE 스트리밍. ChatGPT/Gemini 구독이 있으면 OAuth로 연동해서 별도 키 없이 쓸 수도 있다.

**AI 품질 보증** — 생성 결과를 체크리스트로 자가 검증하고, 미달이면 재생성한다. 검색 QA는 정확성과 유용성을 분리 평가한다. 스트리밍 중에도 반복/이탈을 실시간 감지한다.

**콘텐츠 인텔리전스** — 자동 태깅, 관련 노트 발견, 잊혀진 노트 재발견. 노트 간 관계를 AI가 자동으로 파악한다.

**멀티모달** — PDF에서 텍스트 추출(pymupdf). 이미지에서 OCR(AI Vision 또는 PaddleOCR-VL). 추출 결과는 검색 인덱스에 자동 반영.

**Synology 연동** — NoteStation과 양방향 동기화. 이미지 첨부파일 표시. NSX 파일 직접 임포트. NAS 없이도 로컬 노트 생성 가능.
...
설치 스크립트가 환경 설정, 컨테이너 실행, DB 마이그레이션을 자동으로 처리한다. 완료 후 http://localhost:3000 에서 회원가입.

> 비대화형: `bash install.sh -y` — 기본값으로 바로 설치. 나중에 웹 UI 설정에서 키 추가 가능.
...
NAS 없이도 NSX 임포트나 로컬 노트 생성으로 사용 가능. AI 키 없이도 검색과 노트 관리는 동작.
...
[AGPL-3.0](LICENSE) — 네트워크 서비스 제공 시에도 소스 공개 의무 적용.
