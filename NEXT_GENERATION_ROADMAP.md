...
### 3-6. 노트북 카테고리 시스템 — Tier 1 (UI-3.8) ✅ v2.0.0 선행 구현

| 항목 | 내용 |
|------|------|
| **문제** | 노트북이 단순 폴더. 카테고리별 특화 기능 없음 |
| **해결** | 12종 프리셋 카테고리 (연구 6 + 라이프스타일 6) + AI 인지 도구 통합 |

> ✅ **v2.0.0에서 선행 구현 완료** (원래 v3.1.0 계획)

**구현 내역**:
- DB: `notebooks.category` 컬럼 (마이그레이션 022)
- Backend: `constants.py` — 12종 프리셋 (value, ko, en, color, prompt, extraction_hints, search_boost_terms)
- Backend: `api/notebooks.py` — 카테고리 CRUD + 필터, `api/settings.py` — 카테고리 동적 설정
- Backend: `api/ai.py` — `_get_category_prompt()` AI 프롬프트·힌트·부스트 자동 주입
- Frontend: `lib/categories.ts` — CategoryPreset + useCategories() 훅
- Frontend: `CategorySettingsSection.tsx` — 카테고리 CRUD 편집 UI
- Frontend: Settings "Category" 탭, Notebooks 페이지 카테고리 표시

**12종 카테고리 프리셋**:
| # | 연구 카테고리 | 라이프스타일 카테고리 |
|---|-------------|-------------------|
| 1 | 🔬 labnote (실험 노트) | 📔 diary (일기) |
| 2 | 📋 daily_log (일일 기록) | ✈️ travel (여행) |
| 3 | 🤝 meeting (회의록) | 🍳 recipe (레시피) |
| 4 | 📖 sop (표준운영절차) | 💪 health (건강) |
| 5 | 🧪 protocol (프로토콜) | 💰 finance (재무) |
| 6 | 📚 reference (참고문헌) | 🎨 hobby (취미) |
...
### 4-5. 외부 콘텐츠 캡처 (Phase 4-3) ✅ v1.4.0 선행 구현
...
> ✅ **v1.4.0에서 선행 구현 완료** (원래 v3.2.0 계획)

**구현 내역**:
- URL 북마크 → readability-lxml + html2text 자동 추출
- arXiv Atom API, PubMed NCBI E-utilities 학술 메타데이터 파싱
...
### 5-8. 평가 인프라 (Phase 5-1, 5-2, 5-3) ✅ v2.0.0 선행 구현
...
> ✅ **v2.0.0에서 선행 구현 완료** (원래 v4.0.0 계획)

**구현 내역**:
- **5-8a. A/B 평가**: `services/evaluation/` (framework, test_generator, scorer, report) — 합성 테스트 데이터 생성 + 모델 비교 + 자동 채점 + 백그라운드 실행
- **5-8b. 검색 메트릭**: `services/search_metrics.py` — fire-and-forget 이벤트 기록, 일별 검색량/평균 소요 시간/0-result 비율/클릭률 집계
- **5-8c. 피드백 루프**: `services/feedback_service.py` — 검색 👍👎 + AI ★1-5 + 기간별 요약
- DB: `search_events`, `search_feedback`, `ai_feedback`, `evaluation_runs` 테이블 (마이그레이션 024)
- Frontend: Admin 3개 탭 (Metrics, Feedback, Evaluation) + useEvaluation, useFeedback 훅
...
| 단계 | 마이그레이션 | 시점 | 상태 |
|------|------------|------|------|
| 1 | `022_add_notebook_synology_id_and_category.py` — `notebooks.category` 컬럼 추가 | v2.0.0 | ✅ 완료 |
| 2 | 카테고리 프리셋: `constants.py` + Settings 동적 관리 (category_configs 테이블 대신) | v2.0.0 | ✅ 완료 |
| 3 | `feature_gates` 테이블 + 초기 기능 매트릭스 시드 | v3.1.0 | 🔲 |
