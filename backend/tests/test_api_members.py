...
class TestLogin:
    ...
    async def test_login_success(self):
        ...
        user = _make_user(1, "login@example.com", "Login User", hash_password("securepassword123"))
        org = _make_org(1, "Login Org", "login-org-test")
        membership = _make_membership(1, 1, 1, MemberRole.OWNER, datetime.now(UTC))

        mock_result = MagicMock()
        mock_result.scalar_one = MagicMock(return_value=org)
        mock_db.execute = AsyncMock(return_value=mock_result)

        with (
            patch("app.api.members.get_user_by_email", new_callable=AsyncMock, return_value=user),
            patch("app.api.members.verify_password", return_value=True),
            patch("app.api.members.get_user_memberships", new_callable=AsyncMock, return_value=[membership]),
        ):
            from app.database import get_db
    ...
    async def test_login_invalid_password(self):
        ...
        mock_db = AsyncMock()
        user = _make_user(1, "wrongpass@example.com", "Wrong Pass User", "hashed")

        with (
            patch("app.api.members.get_user_by_email", new_callable=AsyncMock, return_value=user),
            patch("app.api.members.verify_password", return_value=False),
        ):
            from app.database import get_db
    ...
    async def test_login_nonexistent_user(self):
        ...
        with patch("app.api.members.get_user_by_email", new_callable=AsyncMock, return_value=None):
            from app.database import get_db
